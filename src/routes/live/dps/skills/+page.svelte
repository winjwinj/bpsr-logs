			...
			<tbody>
					...
					<tr class="h-7 px-2 py-1 text-center">
						...
						<td
							...
							style="background-color: {getClassColor(
								dpsSkillBreakdownWindow.inspectedPlayer.className
							)}; width: 100vw;"
						></td>
					</tr>
				{/each}
				{#each dpsSkillBreakdownTable.getRowModel().rows as row, i (row.id)}
					<tr class="h-7 px-2 py-1 text-center">
						...
						<td
							class="absolute left-0 -z-1 h-7"
							style="background-color: {`color-mix(in srgb, ${getClassColor(dpsSkillBreakdownWindow.inspectedPlayer.className)} 80%, white ${i % 2 === 0 ? '50%' : '20%'})`}; width: {(row
								.original.totalValue /
								dpsSkillBreakdownWindow.topValue) *
								100}%;"
						></td>
					</tr>
				{/each}
