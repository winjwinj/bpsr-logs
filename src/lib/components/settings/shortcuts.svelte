<script lang="ts">
	...
	let inputs: BaseInputs = [
		{ id: 'toggleLiveMeter', label: 'Hide/Show DPS Meter' },
		{ id: 'toggleClickthrough', label: 'Toggle Clickthrough' },
		{ id: 'resetEncounter', label: 'Reset Encounter' },
		{ id: 'showDpsTab', label: 'Show DPS Tab' },
		{ id: 'showHealTab', label: 'Show Heal Tab' },
		{ id: 'hardReset', label: 'Hard Reset' }
	];
</script>
	...
	<Card.Root>
		<Card.Content class="space-y-2">
			{#each inputs as input (input.id)}
				<div class="flex flex-row items-center justify-between gap-3 py-0.5">
					<div class="text-sm leading-tight font-medium">{input.label}</div>
					<Button
						variant="outline"
						class="text-sm"
