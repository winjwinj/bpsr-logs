...
<header
	...
	class="sticky top-0 flex h-7 w-full items-center justify-between bg-neutral-900/80 px-1"
>
	<!-- Left side -->
	<span>
		<button
		...
		<span
			...
			<span {@attach tooltip(() => headerInfo.totalDmg.toLocaleString())}
				><AbbreviatedNumber num={Number(headerInfo.totalDmg)} /></span
			></span
		>
		<span
			><span {@attach tooltip(() => 'Total Damage per Second')}>T.DPS</span>
			<span {@attach tooltip(() => headerInfo.totalDps.toLocaleString())}
	...
	<span class="flex gap-1">
		...
		<button
			...
			{@attach tooltip(() => 'Clickthrough')}><PointerIcon /></button
		>
		<button onclick={() => openSettings()} {@attach tooltip(() => 'Settings')}
			><SettingsIcon /></button
		>
		<button onclick={() => appWindow.hide()} {@attach tooltip(() => 'Minimize')}
			><MinusIcon /></button
		>
	</span>
</header>
